...
def create_document_store_files():
    document_store_dir = "aj_mas/document_store"
    create_directory(document_store_dir)
...
"""
    write_file(os.path.join(document_store_dir, "__init__.py"), init_content.strip())

    # document_store.py
    document_store_content = """
...
class DocumentStore(ABC):
    ...
    def retrieve_relevant_content(self, query: str, top_k: int = 3) -> List[Tuple[str, str]]:
        pass
"""
    write_file(os.path.join(document_store_dir, "document_store.py"), document_store_content.strip())

    # fasttext_chroma_store.py
    fasttext_chroma_store_content = """
import os
import hashlib
import fasttext
import chromadb
...
"""
    write_file(os.path.join(document_store_dir, "fasttext_chroma_store.py"), fasttext_chroma_store_content.strip())

if __name__ == "__main__":
    create_document_store_files()
    print("AJ_MAS document store files created successfully!")
